...
Follow this section to integrate your AWS services with {{ short_name }}.
...
- A {{ short_name }} user account with the **awsiottenant** role.

    !!! important
        It's recommended to create a {{ short_name }} user account dedicated exclusively for the integration. Note that the integration will work correctly only until the user with the **awsiottenant** role exists in {{ short_name }} and the AWS CloudFormation stack is appropriately configured.
...
## Create a {{ short_name }} REST user

To start integrating AWS with {{ short_name }}, you first need to create a user account that will be used to authorize and authenticate API calls from AWS in {{ short_name }}. To do that:

1. Go to your {{ short_name }} account and from the **Administration** menu, select **Users management**.
...
2. Enter your AWS Endpoint Name in {{ short_name }}:
    ...
    - In {{ short_name }}, go to the **Profiles** tab of your integration group and paste the result as the value for `AWSdataPlaneEndpointAddress`.
...
0. Choose a name for the stack and provide the parameters:
    ...
    !!! important
        The credentials you provide at this point should belong to the user with access to the {{ short_name }} group which stores the AWS configuration set in a previous step.
    ...
    - **coioteDMrestUri** - URL address and port of your {{ short_name }} installation. The port should always be `8088` to enable proper mTLS-based authentication.
...
To remove the integration of AWS and {{ short_name }}, follow the following steps:
