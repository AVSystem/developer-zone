...
The {{ short_name }} and Azure IoT Hub integration lets you create custom use cases with data visualization. See the video and have a sneak peek at the possibilities that the {{ short_name }} - Azure IoT Hub integration offers you. In the tutorial, you will see how to leverage the integration to create an air quality monitoring in just a few steps.
...
- An active {{ short_name }} account. Please refer to [{{ short_name }} home page](https://www.avsystem.com/products/coiote-iot-device-management-platform/) for details on how to get it.
...
Once the deployments are complete, go to {{ short_name }} to set up the Azure IoT Hub extension.
...
## Adding and connecting LwM2M air quality meter simulators to {{ short_name }} and Azure IoT Hub
...
2. Go to {{ short_name }} and sync the previously added devices:
...
4. Go back to {{ short_name }} and in **Device inventory**, check if the devices have registered to the platform and if their data model has been updated.
...
### From {{ short_name }} to Azure IoT Hub

1. In your {{ short_name }} account, go to **Device inventory**, select a device.
...
### From Azure IoT Hub to {{ short_name }}

!!! note
    To read more about how the Device Twins work in the {{ short_name }} - Azure IoT Hub integration, please refer to [the LwM2M Mappings section](/Concepts/LwM2M_mappings/#lwm2m-readable-and-writable-resources).

1. In your Azure IoT hub, select **IoT devices**, click one of your added devices and select the **Device Twin** panel.
2. To change the ``Lifetime`` resource in {{ short_name }}, you need to modify the relevant Device Twin desired property.
...
3. The value of the resource should now be changed in the Device Twin reported properties as well as in the {{ short_name }} Objects panel, in the ``Lifetime`` resource of the ``1 LwM2M Server`` object.

## Passing telemetry to Azure IoT Hub

### Setting group value tracking on resources in {{ short_name }}

1. In {{ short_name }}, go to **Device inventory** and use the search option to display your air quality meter devices. Then, click the **Add to group** icon.
