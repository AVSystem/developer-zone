...
Integrate your ESP32-based device with Quectel BG96 module to manage it via {{ short_name }}.
...
- A user with access to the [Coiote IoT Device Management platform]({{ site_link }}).
...
0. Open the **menuconfig** with the `idf.py menuconfig` command, navigate to **Component config -> anjay-esp32-client**.
    ...
    - In **Client options**, provide device credentials and Server URI:
        ...
        - **Server URI** - the address and port of your {{ short_name }} installation, e.g. ``coaps://eu.iot.avsystem.cloud:5684``
...
To connect your ESP32-DevKitC to the {{ site_name }}, use your access to a {{ short_name }} installation, or register at {{ site_link }}/ to get access.
