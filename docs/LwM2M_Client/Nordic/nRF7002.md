...
Integrate your Nordic nRF7002 board.

## Prerequisites

- The nRF7002 board with a USB cable.
...
0. To program the board, go through the whole process of **flashing nRF** [described here](https://developer.nordicsemi.com/nRF_Connect_SDK/doc/latest/nrf/ug_nrf7002_gs.html).
0. Use the nRF Connect Programmer with the downloaded `.hex` file and execute steps from the [Updating the application firmware](https://developer.nordicsemi.com/nRF_Connect_SDK/doc/latest/nrf/ug_nrf7002_gs.html#updating-the-application-firmware) section.
...
0. Connect the nRF7002 board to a USB port of your machine.
...
0. Compile the project for **nRF7002**:

    ```
    cd Anjay-zephyr-client/demo
    west build -b nrf7002dk_nrf7002ns
    ```

0. Find the `merged.hex` file under the `build/zephyr` directory in the project folder.

0. Use the nRF Connect Programmer to flash the created `.hex` file onto the board. For more information, follow the [updating the application firmware](https://developer.nordicsemi.com/nRF_Connect_SDK/doc/latest/nrf/ug_nrf7002_gs.html#updating-the-application-firmware) documentation.
...
!!! note
    **nRF7002** uses the same radio for GPS and LTE connectivity. GPS signal is
