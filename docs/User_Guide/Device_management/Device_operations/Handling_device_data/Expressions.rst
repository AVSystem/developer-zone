...
Usages of expressions include:

 * :ref:`Migration rules <UG_MDG_Migrations>` - expressions are used to define the migration condition and destination group.
 * Task configurations (see :ref:`Xml task editor`) - a value of every task configuration attribute can be an expression. For example, here is a simple XML task that writes some simple message to device logs. The message is defined using an expression:
 ...
 * Arithmetic operators ``+``, ``-``, ``*``, ``/`` and ``%`` (modulo)
  ...
  .. warning:: Division operator ``/`` used on integral values (``Int`` and ``Long``) is an *integer* division, i.e. it will truncate the value down and strip the decimal part. For example ``3/2`` will evaluate to ``1``. If you need the division to work on decimal values, convert at least one of the operands to ``Double`` value, e.g. ``3.0/2`` or ``3.toDouble/2``.
 ...
 * Fallback value operator ``?`` The fallback value operator takes its left-hand-side and tries to evaluate it. If evaluation succeeded, it will simply return the value of left-hand-side. If the evaluation failed or the value was ``null`` a fallback value will be returned - the right-hand-side operand. Fallback value operator handles following types of failures:
  ...
  * Someone tried to access array or list element at an index out of its bounds
   ...
   .. warning:: The ``?`` operator has very high precedence - higher than any other operator. Use parentheses to enforce precedence you want.

.. _Expression collections:
...
Common way of obtaining a list of strings is to split some text value:
 ...
 * Every numerical data type has the following conversions available: ``toInt``, ``toLong``, ``toDouble``, ``toChar``
  ...
  .. warning:: Conversions from broader to narrower numeric types may cause loss of precision and overflow.
...
.. image:: images/exp_editor.*
   :align: center

#. Expression editor text field. Light red background indicates that a currently typed expression is invalid (in this
   case incomplete). You can hover your mouse over the editor and more detailed error message will be displayed in tooltip.
#. API suggestions. In the example above, the API of the ``String`` data type is suggested (because ``sv.something`` is
...
#. API documentation. Hover your mouse over selected suggestion to display documentation in tooltip.

You can also use :ref:`UIExpressions Sandbox` to test your expressions.

.. _Expression common API:
